...
def vector_embedding():
    ...
    if 'vectors' not in st.session_state:
        ...
        if os.path.exists('objectbox'):
            shutil.rmtree('objectbox', ignore_errors=True)
        st.session_state.vectors = ObjectBox.from_documents(st.session_state.final_documents, st.session_state.embeddings, embedding_dimensions=384, db_directory='objectbox')



if st.button('Embedd Documents'):
    with st.spinner("Embedding documents into ObjectBox database..."):
        vector_embedding()
    st.success('ObjectBox Database is ready. You can now enter your question.')

user_input = st.text_input('Enter your question from documents')

if user_input:
    if 'vectors' not in st.session_state:
        st.warning("Please click the 'Embedd Documents' button first to initialize the database.")
    else:
